#!/bin/bash
# Sunshine global_prep_cmd "do" — a Moonlight/MoonDeck client connected.
# 1. create a virtual display at the client's requested resolution (dynamic,
#    never hardcoded — read from SUNSHINE_CLIENT_WIDTH/HEIGHT each time)
...
# XAUTHORITY goes stale in a long-lived Sunshine process across compositor
# restarts (a new xauth_XXXXXX file is generated each session) — resolve it
# dynamically every run rather than relying on Sunshine's own inherited env.
...
setsid systemd-inhibit --what=idle:sleep:handle-lid-switch --who=sunshine \
