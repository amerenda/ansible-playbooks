...
# reaching in.
#
# is_couch_mode guard added 2026-08-31: this unit is PartOf=plasmalogin.service,
# so it re-fires any time plasmalogin restarts, not just at boot - including
# a spurious mid-session relogin (the documented CachyOS plasmalogin
# double-login bug) while the desk user is actively in couch mode. Without
# this guard it disabled the couch TV output outright during a live gaming
# session with no warning, mid-game, for no CEC-related reason at all -
# confirmed live 2026-08-31. The sibling script above already had this
# guard; this one never did.
set -euo pipefail
...
if is_couch_mode; then
    echo "force-desk-primary-greeter: couch mode active, skipping desk-primary enforcement" >&2
    exit 0
fi
