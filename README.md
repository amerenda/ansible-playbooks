...
| `setup-cachyos.yml` | Full CachyOS/Arch gaming+GPU workstation bootstrap for a rebuilt `archlinux`: reuses `setup-archlinux-k3s.yml` / `setup-gpu-runtime.yml` / `setup-archlinux-komodo.yml` / `k3s-agent.yml`, adds ROCm (rocminfo/rocm-smi), gaming (GameMode/MangoHud/Lutris/Proton-GE), dotfiles + SSH keys from BWS, Sunshine, joystick-notify, restic-to-GCS backup | `cachyos_workstations`; **`--limit` must include `localhost`** (e.g. `--limit "archlinux,localhost"`) or the BWS fetch play is silently skipped; `k3s-agent.yml` self-gates and won't disrupt an already-joined agent |
